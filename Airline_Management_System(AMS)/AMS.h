...
public:
    Airline() = default;
    Airline(const string &comp,const string &line,const string &dep,const string &dest,int maxp,int re):
        Company(comp),LineNo(line),Departure(dep),Destination(dest),Maxpassenger(maxp),RemainTickets(re) {}
    string Company;
    string LineNo;
    string Departure;
    string Destination;
//    string Airplane;
//    Time DepartureTime;                          ,const string &airp,Time depa,Time desti
//    Time DestinationTime;  添加回来时记得改构造函数 ,Airplane(airp),DepartureTime(depa),DestinationTime(desti)
    int Maxpassenger;
    int RemainTickets;
